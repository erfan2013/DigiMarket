// backend/controller/user/meController.js
...
exports.updateAvatar = async (req, res) => {
  try {
    ...
    if (!isConfigured) {
      return res.status(503).json({ success:false, error:true, message:"Cloudinary not configured" });
    }

    const user = await userModel.findById(req.userId);
    if (!user) return res.status(404).json({ success:false, error:true, message:"User not found" });

    // آپلود استریمی به Cloudinary
    ...
    user.avatar = result.secure_url; // فقط URL
    await user.save();
...
//     user.avatar = result.secure_url;        // فقط URL لازم داری
//     await user.save();
